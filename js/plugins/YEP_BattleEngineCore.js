...
BattleManager.displayStartMessages = function() { //Adam Szlamp edited to include number of monsters 
    if (eval(Yanfly.Param.BECEmergeText)) {
      var enemyNum = 0;      
      $gameTroop.enemyNames().forEach(function(name) {        
        if($gameTroop._enemies.length > 0){
          $gameTroop._enemies.forEach(function(foe){
            var enemyName = foe.originalName();
            if(enemyName === name){
              enemyNum++;
            }
          })
          if (enemyNum == 1){
            $gameMessage.add(TextManager.emerge.format("A ", name, "", " has"));
          }
          else
          {            
          var enemyNumString = enemyNum.toString() + " ";
          switch(name){
            case "Wolf":
              name = "Wolve";
              break;
            ...
            case "Snowman":
              name = "Snowmen";
              $gameMessage.add(TextManager.emerge.format(enemyNumString, name, ""," have"));
              return;
              break;
            default:
              console.log(name);
          ...
          $gameMessage.add(TextManager.emerge.format(enemyNumString, name, "s"," have"));          
          }
        }
          else{
            $gameMessage.add(TextManager.emerge.format("A ", name, "", " has"));
...
Game_Battler.prototype.performDamage = function() {
    ...
    this.requestEffect('blink'); //Adam Szlamp Edit
    this.performFlinch();
};
